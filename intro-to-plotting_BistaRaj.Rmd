...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = species)) + #change the size 
  geom_point(size = 2) +
  theme_minimal()+
  scale_colour_manual(values = c("blue", "orange", "gray")) #change the color
...
ggplot(penguins, aes(x = bill_depth_mm, y = flipper_length_mm, color = species)) + #change the size 
  geom_point(size = 2) +
  theme_minimal()+
  scale_colour_manual(values = c("blue", "orange", "gray")) +
...
```
It doesn't produce the scatterplot because as species, bodymass is not a character but a continuous numerical variable. Instead, if we color the plots by other character (categorical variables) like island and sex, scatterplot is generated.
